...
async def axi4lite_test(dut):
    ...
    cocotb.log.info(f"WRITE: Addr=0x2 Data=0x{int(dut.uio_in.value):02X}")

    # ---------------- READ ----------------
    await Timer(20, units="ns")
    dut.ui_in.value = (2 << 4) | (1 << 3)  # read_addr=2, start_read=1
    await Timer(10, units="ns")
    dut.ui_in.value = (2 << 4)              # deassert start_read

    while dut.uo_out.value[0] != 1:
        await RisingEdge(dut.clk)

    read_data = int(dut.uio_out.value)
    cocotb.log.info(f"READ: Addr=0x2 Data=0x{read_data:02X}")
